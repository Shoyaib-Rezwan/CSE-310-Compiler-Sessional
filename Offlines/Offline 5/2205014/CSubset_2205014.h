...
public:
    int getErrorCnt() const { return errorCnt; }
    ...
    any visitFuncDefNoParam(CSubset_2205014Parser::FuncDefNoParamContext *ctx) override
    {
        ...
        vector<string> params = s->getParamTypes();
        if (checkParamNumbers && params.size() != 0)
        {
            errMsg = "Error at line " + to_string(ctx->getStart()->getLine()) + ": Total number of arguments mismatch with declaration in function " + ID + "\n\n";
            errorFile << errMsg;
            logFile << errMsg;
    ...
    any visitUnaryAdd(CSubset_2205014Parser::UnaryAddContext *ctx) override
    {
        ...
        exprTypes.put(ctx, exprTypes.get(ctx->unary_expression())); // Propagate type upward
    ...
    any visitUnaryNot(CSubset_2205014Parser::UnaryNotContext *ctx) override
    {
        ...
        exprTypes.put(ctx, "INT"); // Logical NOT evaluates to an integer
    ...
    any visitFactorId(CSubset_2205014Parser::FactorIdContext *ctx) override
    {
        ...
        // Save outer arguments before processing inner ones
        vector<string> savedArgTypes = argTypes;
        argTypes.clear();

        string argument_list = any_cast<string>(visit(ctx->argument_list()));

        // Capture inner arguments, then restore outer arguments
        vector<string> currentArgTypes = argTypes;
        argTypes = savedArgTypes;
        ...
        {
            vector<string> params = s->getParamTypes();
            if (params.size() != currentArgTypes.size())
            {
            ...
            else
            {
                ...
                {
                    if (!(equalsIgnoreCase(params[i], "float") && equalsIgnoreCase(currentArgTypes[i], "int")) && !equalsIgnoreCase(params[i], currentArgTypes[i]))
                    {
    ...
    any visitFactorParen(CSubset_2205014Parser::FactorParenContext *ctx) override
    {
        ...
        exprTypes.put(ctx, exprTypes.get(ctx->expression())); // Propagate type upward
    ...
    any visitStmtPrintErr(CSubset_2205014Parser::StmtPrintErrContext *ctx) override
    {
        ...
        string errMsg2 = "Error at line " + to_string(ctx->getStart()->getLine()) + ": syntax error, missing ';' after expression '" + matchStr + "'\n\n";
        errorFile << errMsg2;
        logFile << errMsg2;
        errorCnt++;
