...
public:
    ...
    any visitDec_lstCommaID2(CSubset_2205014Parser::Dec_lstCommaID2Context *ctx) override
    {
        string declaration_list = any_cast<string>(visit(ctx->declaration_list()));
        string COMMA = ctx->COMMA()->getText();
        string ID = ctx->ID()->getText();
        ...
        string fectRight = fetchExpr(logic_expression);
        string assign = "\tMOV [" + dst + "], EAX\n";
        writeIntoTempFile(fectRight + assign);
        // string assign
        string matchStr = declaration_list + COMMA + ID + ASSIGNOP + logic_expression;
        return matchStr;
    }
    any visitDec_lstID2(CSubset_2205014Parser::Dec_lstID2Context *ctx) override
    {
        string ID = ctx->ID()->getText();
        table->insert(ID, "ID", decLstType);
        if (insGlobalScope)
        ...
        string assign = "\tMOV [" + dst + "], EAX\n";
        writeIntoTempFile(fectRight + assign);
        string matchStr = ID + ASSIGNOP + logic_expression;
        return matchStr;
    }
};
