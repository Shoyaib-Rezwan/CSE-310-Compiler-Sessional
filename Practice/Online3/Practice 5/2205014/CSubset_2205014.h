...
public:
    ...
    any visitStmtTer(CSubset_2205014Parser::StmtTerContext *ctx) override
    {
        string variable = any_cast<string>(visit(ctx->variable()));
        string ASSIGNOP = ctx->ASSIGNOP()->getText();
        string LPAREN = ctx->LPAREN()->getText();
        string logic_expression0 = any_cast<string>(visit(ctx->logic_expression(0)));
        string fetchExpr1 = fetchExpr(logic_expression0);
        string compare = "\tCMP EAX, 0\n";
        int truel = ++labelCounter;
        int falsel = ++labelCounter;
        int exitl = ++labelCounter;
        string jmp = "\tJE .L" + to_string(falsel)+"\n";
        string RPAREN = ctx->RPAREN()->getText();
        string QUESTION = ctx->QUESTION()->getText();
        string logic_expression1 = any_cast<string>(visit(ctx->logic_expression(1)));
        string COLON = ctx->COLON()->getText();
        string logic_expression2 = any_cast<string>(visit(ctx->logic_expression(2)));
        string fetchExpr2 = fetchExpr(logic_expression1);
        string fetchExpr3 = fetchExpr(logic_expression2);
        string true_task = ".L" + to_string(truel) + ":\n" + fetchExpr2 + "\tMOV [" + fetchSrc(variable) + "], EAX\n\tJMP .L" + to_string(exitl)+"\n";
        string false_task = ".L" + to_string(falsel) + ":\n" + fetchExpr3 + "\tMOV [" + fetchSrc(variable) + "], EAX\n\tJMP .L" + to_string(exitl)+"\n";
        string SEMICOLON = ctx->SEMICOLON()->getText();
        writeIntoTempFile(fetchExpr1 + compare + jmp + true_task + false_task + ".L" + to_string(exitl) + ":\n");
        string matchStr = variable + ASSIGNOP + LPAREN + logic_expression0 + RPAREN + QUESTION + logic_expression1 + COLON + logic_expression2 + SEMICOLON;
        return matchStr;
    }
    virtual std::any visitIncopFactor(CSubset_2205014Parser::IncopFactorContext *ctx) override
    {
        string INCOP = ctx->INCOP()->getText();
        string variable = any_cast<string>(visit(ctx->variable()));
        // offline 4
    ...
    virtual std::any visitDecopFactor(CSubset_2205014Parser::DecopFactorContext *ctx) override
    {
        string INCOP = ctx->DECOP()->getText();
        string variable = any_cast<string>(visit(ctx->variable()));
        // offline 4
        string src = "";
        SymbolInfo *s = lookup(variable);
