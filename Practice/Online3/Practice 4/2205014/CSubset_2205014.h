...
public:
    ...
    any visitEXprAdd(CSubset_2205014Parser::EXprAddContext *ctx) override
    {
        string variable = any_cast<string>(visit(ctx->variable()));
        string fetchLeft = "";
        SymbolInfo *s = lookup(variable);
        ...
        if (s)
        {
            ...
            dst = src;
            fetchLeft = "\tMOV EAX, [" + src + "]\n";
        }
        string ADDOP = ctx->ADDOP()->getText();
        bool isPlus = (ADDOP == "+") ? true : false;
        string ASSIGNOP = ctx->ASSIGNOP()->getText();
        string logic_expression = any_cast<string>(visit(ctx->logic_expression()));
        s = lookup(logic_expression);
        string fetchRight = "";
        ...
        {
            fetchRight = "\tMOV EAX, " + logic_expression + "\n";
        }
        string move = "\tMOV EDX, EAX\n";
        string operation = "";
        if (isPlus)
        {
            operation = "\tADD EAX, EDX\n";
        }
        else
        {
            operation = "\tSUB EAX, EDX\n";
        }
        string store = "\tMOV [" + dst + "], EAX\n";
        string push = "\tPUSH EAX\n";
        fetchFromStack = true;
        writeIntoTempFile(fetchRight + move + fetchLeft + operation + store + push);
        string matchStr = variable + " " + ADDOP + ASSIGNOP + " " + logic_expression;
        return matchStr;
    }

    any visitEXprMul(CSubset_2205014Parser::EXprMulContext *ctx) override
    {
        string variable = any_cast<string>(visit(ctx->variable()));
        SymbolInfo *s = lookup(variable);
        string fetchLeft = "";
        ...
        fetchFromStack = true;
        writeIntoTempFile(fetchRight + move + fetchLeft + operation + store + push);
        string matchStr = variable + " " + MULOP + ASSIGNOP + " " + logic_expression;
        return matchStr;
    }
};
