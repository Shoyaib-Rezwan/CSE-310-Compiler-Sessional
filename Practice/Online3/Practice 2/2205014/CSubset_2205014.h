...
public:
    ...
    any visitStmtPrint(CSubset_2205014Parser::StmtPrintContext *ctx) override
    {
        ...
        string SEMICOLON = ctx->SEMICOLON()->getText();
        print_called = false;
        string matchStr = PRINTLN + LPAREN + argument_list + RPAREN + SEMICOLON;
        ...
        // offline 4
        // string fetchData = "";
        // string callPrint = "\tCALL print_number\n";
        // for (string exp : argStrs)
        // {
        //     SymbolInfo *s = lookup(exp);
        //     if (s)
        //     {
        //         string src = "";
        //         if (s->getStkOffset())
        ...
        //             src = exp;
        //         }
        //         fetchData = "\tMOV EAX, [" + src + "]\t\t; Line " + to_string(ctx->getStart()->getLine()) + "\n";
        //     }
        ...
        //     }
        //     writeIntoTempFile(fetchData + callPrint);
        //     printfUsed = true;
        // }
        // offline 4

        string message = "Line " + to_string(ctx->getStart()->getLine()) + ": statement : PRINTLN LPAREN arguments RPAREN SEMICOLON\n\n";
        message = message + matchStr + "\n\n\n";
        writeIntoLogFile(message);
        return matchStr;
    }
    ...
    any visitArgsArgs(CSubset_2205014Parser::ArgsArgsContext *ctx) override
    {
        ...
        if (print_called)
        {
            SymbolInfo *s = lookup(logic_expression);
            string fetchData = "";
            string call_print = "\tCALL print_number\n";
            if (s)
            {
                string src = "";
                if (s->getStkOffset())
            ...
            writeIntoTempFile(fetchData + call_print);
        }

        argStrs.push_back(logic_expression);
