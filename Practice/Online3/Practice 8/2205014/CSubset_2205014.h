...
public:
    ...
    any visitStmtSwap(CSubset_2205014Parser::StmtSwapContext *ctx) override
    {
        string SWAP = ctx->SWAP()->getText();
        string LPAREN = ctx->LPAREN()->getText();
        string ID1 = ctx->ID(0)->getText();
        string fetchLeft = fetchExpr(ID1);
        string COMMA = ctx->COMMA()->getText();
        string ID2 = ctx->ID(1)->getText();
        string fetchRight = fetchExpr(ID2);
        string moveEAX = "\tMOV EDX, EAX\n";
        string assign1 = "\tMOV [" + fetchSrc(ID1) + "], EDX\n";
        string assign2 = "\tMOV [" + fetchSrc(ID2) + "], EAX\n";
        writeIntoTempFile(fetchRight + moveEAX + fetchLeft + assign1 + assign2);
        string RPAREN = ctx->RPAREN()->getText();
        string SEMICOLON = ctx->SEMICOLON()->getText();
        string matchStr = SWAP + LPAREN + ID1 + COMMA + ID2 + RPAREN + SEMICOLON;
