...
void handleCommands(int bucketSize, SymbolTable *symbolTable)
{
    ...
    while (getline(cin, line))
    {
        ...
        if (cmd == "I")
        {
            ...
            int uniqueNumber = -1;
            if (!symbolTable->insert(key, type, uniqueNumber))
                cout << "\t'" << key << "' already exists in the current ScopeTable\n";
            if (uniqueNumber != symbolTable->getCurrentScope() && uniqueNumber!=-1)
            {
                cout << "\tWarning: '" << key << "' shadows a declaration in ScopeTable# " << uniqueNumber << '\n';
            }
        }
