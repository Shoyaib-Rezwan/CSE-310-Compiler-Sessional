...
void handleCommands(int bucketSize, SymbolTable *symbolTable)
{
    ...
    while (getline(cin, line))
    {
        ...
        if (!(cmd == "I" || cmd == "L" || cmd == "S" || cmd == "E" || cmd == "Q" || cmd == "P" || cmd == "D" || cmd == "N"))
        {
            continue;
        }
        if (cmd == "P")
        ...
        if (cmd == "N")
        {
            if (stream >> key)
            {
                cout << "\tNumber of parameters mismatch for the command N\n";
                cmdNo++;
                continue;
            }
            int currentCount = 0, currentScope = symbolTable->getCurrentScope(), totalCount = 0;
            totalCount = symbolTable->countSymbols(currentCount);
            cout << "\tCurrent ScopeTable# " << currentScope << " contains " << currentCount << " symbol(s)\n";
            cout << "\tTotal visible symbols (including outer scopes): "<<totalCount<<'\n';
        }
        cmdNo++;
    }
}
