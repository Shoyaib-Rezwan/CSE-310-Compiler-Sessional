...
public:
    ...
    int merge()
    {
        ...
        if (current != nullptr && current->getUniqueNumber() != 1)
        {
            ...
            current = current->getParent();
            uniqueNumber = temp->getUniqueNumber();
            SymbolInfo **buckets = temp->getBuckets();
            for (int i = 0; i < temp->getBucketSize(); i++)
            {
                SymbolInfo *temp2 = buckets[i];
                while (temp2 != nullptr)
                {
                    if(!current->insert(temp2->getName(), temp2->getType()))
                    {
                        cout << "\t'" << temp2->getName() << "' already exists in ScopeTable# "<<current->getUniqueNumber()<<"\n";
                    }
                    temp2=temp2->getNext();
                }
            }
