...
void handleCommands(int bucketSize, SymbolTable *symbolTable)
{
    ...
    while (getline(cin, line))
    {
        ...
        if (!(cmd == "I" || cmd == "L" || cmd == "S" || cmd == "E" || cmd == "Q" || cmd == "P" || cmd == "D" || cmd == "M"))
        {
            continue;
        }
        if (cmd == "P")
        ...
        else if (cmd == "M")
        {
            if (stream >> key)
            {
                cout << "\tNumber of parameters mismatch for the command M\n";
                cmdNo++;
                continue;
            }
            int uniqueNumber = symbolTable->merge();
            if (uniqueNumber != -1)
                cout << "\tScopeTable# " << uniqueNumber << " merged into ScopeTable# " << symbolTable->getCurrentScope() << '\n';
        }
        cmdNo++;
    }
}
